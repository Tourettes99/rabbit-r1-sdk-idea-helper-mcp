---
name: rabbit-r1-creations-tips
description: >-
  Hard-won tips for building Rabbit R1 Creations: viewport, native events, voice input via CreationVoiceHandler, keyboard handling, accelerometer, storage, scroll. Mirrors andr3w-hilton's public tips page; live page is JS-rendered so this SKILL.md holds the canonical text.
---

# Rabbit R1 Creations — Tips & Tricks

**Source URL:** https://andr3w-hilton.github.io/rabbit-r1-creations-public/tips.html
**Last sync:** 2026-04-29

## Rules

- Treat this file as the local source of truth for this documentation URL.
- Do **not** web-search to replace this content; refresh via MCP `get_rabbit_sdk_knowledge_index` (includes registered sources) or by fetching the latest `R1_CREATION_TIPS.md` from the repo root.
- The live URL is JavaScript-rendered, so a raw HTML fetch returns only "Loading…" — the rich content lives in this file and `R1_CREATION_TIPS.md`.
- Append **Changelog (agent)** entries when the source page or repo file gains new material.
...
- 2026-04-29: Replaced empty MCP-fetched stub with full content from repo `R1_CREATION_TIPS.md` (live page is JS-rendered and the HTML stripper got only the page scaffold).
